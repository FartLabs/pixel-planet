---
title: Installation
description: Learn how to add Pixel Planet components to your React project.
---

import CliCommandCode from "@/components/markdown/cli-command-code/cli-command-code.astro"
import { SERVER_URL } from "@/data/env"

Learn how to add Pixel Planet components to your React project. Because Pixel Planet is hosted on a custom GitHub Pages registry, you have two ways to install it using the `shadcn` CLI.

### Prerequisites

Before installing, ensure your project meets these requirements:

- **React**: A React-based project (e.g., Next.js, Vite).
- **React Three Fiber**: Install `@react-three/fiber` and `three` to render the 3D components.

<CliCommandCode action="install" command="@react-three/fiber three" />

- **Tailwind CSS**: The components use Tailwind for positioning and styling.

### Add components

The most reliable way to install a component from a custom third-party registry without modifying your project configuration is to use the absolute URL to the component's generated JSON definition.

Run the following command in your terminal:

<CliCommandCode
  action="run"
  command={`shadcn@latest add ${SERVER_URL}/r/pixel-planet.json`}
/>

_This commands the CLI to fetch the component's code and dependencies directly from the repository's build output folder (`/r`)._
